...
1. **§17 longtask-contract skill**（`skills/longtask-contract/SKILL.md`）：
   ...
   故障速查。MANIFEST.json 索引让工具链（agentskills / DSH 等）可发现。
...
attempt/succeeded`。DSH 客户端的 MCP server 注册不在协议范围——每个
...
- claims：`integration_real_dsh` 不在 schema enum 中——改 `manual_review`（第三方
