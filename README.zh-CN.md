...
2. **挑一个 Runner。** 时间到了，它从你批准过的 Agent 列表里（Codex / Claude Code / agent-cli / 任何有 CLI 的工具）挑一个合适的。
...
src/longtask/             参考实现，Python 3.11+，零运行时依赖
  ...
  adapters/               怎么包一个 CLI Runner（Codex / Claude / agent-cli / ...）
  rpc/                    JSON-RPC 控制面
...
examples/                 真实运行归档（不要改 —— 是审计证据）
  agent-cli-model-provider-run/        第一次端到端（DeepSeek Harness + model-provider-M2.7）
  agent-cli-model-provider-run-v2/     同任务，开启临时上下文 + verifier
