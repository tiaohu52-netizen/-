"""Continuity 字段（SPEC §6.1 continuity + §11.1/§11.2/§11.3）。

P2 起独立模块。本期承载 §6.1 列出的 4 字段（checkpoint_max_age_minutes /
checkpoint_on_material_change / recovery_grace_minutes / capsule_max_tokens）
与最小校验；Goal Capsule 9 元组的具体落库结构与 reconcile 四分支留 P3。
"""
...
class Continuity:
    """连续性配置（SPEC §6.1 continuity）。

    checkpoint_max_age_minutes: 由 executor 在发生实质进展 / 收 checkpoint 请求
    / 即将退出 / 超过该时间未提交时提交结构化 checkpoint（§11.2）。
    checkpoint_on_material_change: 仅在实质变化时写 checkpoint；False 表示定
    期写（P3 才真正使用）。
    recovery_grace_minutes: daemon 重启后 reclaim 的 grace window（§11.3）。
    capsule_max_tokens: Goal Capsule 的 token 上限（P3）。
    """
