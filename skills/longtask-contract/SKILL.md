...
**两条硬约定（模型第一次用就踩过的坑，见 `examples/dsh-dogfood-v5/`）**：
...
| 跨进程轮询丢句柄 | daemon 续命依赖持有 Popen；headless harness 见 `examples/dsh-minimax-run-v2/` 已知缺口 |
| 执行者 succeeded 不被识别 | dsh node 主进程是 Popen 持有，但内部 worker turn/end 才会让主进程退——需要总控补录 attempt/succeeded 或 harness 在 stdout 输出结构化 attempt/finished 事件 |
...
- `examples/dsh-minimax-run/`（v1 真实运行，暴露了上下文/verifier 缺口）
- `examples/dsh-minimax-run-v2/`（v2 真实运行，缺口补上后）
