...
**本协议在体系结构上是一个外置 harness，搭载于各 harness 之上，而非某个 harness 的插件。** 现有 Agent 应用（DSH、Codex、Hermes、Qwen 等）本身就是完整的 harness：它们管理模型上下文、沙箱、工具集、审批和会话生命周期。远期任务协议不从属其中任何一个；它把这一批 harness 统一抽象成可替换的执行资源池，自己只做一件事：把一份持久合同推进到底。
...
│   DSH bridge │ Codex subprocess │ Hermes bridge │ …          │
├──────────────────────────────────────────────────────────────┤
...
├──────────────┬──────────────────────────────────────┤
│ 调度层        │ 执行层（适配器，可插拔）                  │
│ ticker/唤醒源 │  DSH / codex / hermes-subagent / …    │
├──────────────┴──────────────────────────────────────┤
...
**冻结区 vs 可修订区**是这份 schema 的关键切分：Deadline、禁令、沙箱边界立合同时就钉死——紧迫性永远不能升级它们（§6.3 硬边界）；目标描述、验收标准、软指引可以改，但每次修改过用户审批门并递增修订号（学 DSH goal 的 `goal/change` 事件 + CAS）。
...
- **blocked** 是唯一的「因问题而停」状态，带 lower-kebab-case 原因码 + 人类可读说明（学 DSH goal 的 GoalBlockReason）。
...
先例佐证：DSH schedule 的 followup() 证明了「往活跃会话注入提醒轮次」这条路是通的，`agent.steer()` 证明了「轮次进行中导向」是通的，Hermes cron 的 ticker 证明了「到点拉起新会话」是通的。升级阶梯 = 把三条既有通道按紧迫度编排。
...
- 与 DSH jobs 的本质差异：DSH 里 owner session 死了任务被取消（状态长在会话里）；本协议 owner 死了换人续跑（状态长在合同里）。
...
agents:
  - id: dsh-web
    ...
    launch:                         # 结构化启动声明，不是可拼接的 shell 字符串
      argv: [dsh, --profile, web, --headless]
      cwd: null
      env_allowlist: [DSH_CONFIG]   # 只透传白名单环境变量
...
| 合同策略 | DSH bridge | process/codex adapter | hermes adapter |
|---|---|---|---|
| `file_effects.mode` | 解析 DSH 沙箱策略；报告 `full/partial` | 映射到该 CLI 的沙箱参数；无法证明则拒接 | 映射到进程沙箱参数；无法证明则拒接 |
...
**编译失败的默认行为是拒接**，拒接事件写入 `log.jsonl`，推动者换下一个候选。绝不静默降级。DSH 的 session-local Schedule 只能作为一次提醒渠道，不能作为 agent 外协议的持久 Deadline 保证。
...
| 执行层 | 各 CLI 的 headless 入口（codex/dsh/qwen 均在 PATH） | 每应用一个薄适配器（~百行级） |
| 推动层 | DSH followup/steer 概念、cron 拉新会话逻辑 | 升级阶梯编排器、租约管理器 |
...
适合 DSH 这类有公开 Agent handle 的应用。插件加载时只做接线，不复制合同状态：
...
DSH 的 `followup` 会排队后续轮次，`steer` 会把可唤醒的输入排入下一步骤；它们不是同一种干预，bridge 必须分别声明和记录。DSH 的 `Schedule` 仍只是 session-local 提醒，不能代替 `longtaskd` 的外部 Deadline 仲裁。
...
可以公开，但 README 必须明确这是协议设计预览，不是已完成的 Agent 编排工具。仓库应只包含脱敏后的规范、schema、示例和 skill，不上传当前 `D:\工作台` 总仓库内容。
...
- DSH bridge 的 followup/steer/interrupt
- Agent 自动发现和能力探测
- 多 Agent 分区并行
- 外部严格 Deadline 唤醒源
- 跨平台安装包和升级/迁移机制
...
3. **DSH Bridge 的宿主接入方式**：控制面和事件订阅可先用本机 JSON-RPC；具体由哪个 DSH profile 承载 bridge、如何发现目标 Agent handle，需在适配器实现前按公开 API 定稿。
