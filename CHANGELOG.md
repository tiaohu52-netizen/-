...
- Claims evidence is re-anchored to the latest verified implementation commit;
  the full 7-gate suite remains green with 561 tests at 81.55% coverage.
...
- Headless harness subprocess lifetime vs. `Popen` handle alignment
  (real-run example in `examples/dsh-minimax-run-v2/`).
- `control/spawn` is vocabulary-only; external RPC to `AttemptRunner`
  not yet exposed.
- DSH / Claude Desktop etc. each need their own `longtask-mcp` registration
  in their MCP client config — out of protocol scope.

### Documentation & examples

- `examples/dsh-minimax-run/` — v1 real execution: workspace artifacts
  delivered by `MiniMax-M2.7-highspeed`; cross-check caught a wrong
  unit-test assertion.
- `examples/dsh-minimax-run-v2/` — same task with §4.1 context + §5.2
