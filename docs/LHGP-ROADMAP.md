...
当前 Developer Preview 发布基线已完成一次全量复验：质量门 7/7 全部通过，585
个测试通过，覆盖率 81.75%；claims 42 条（41 条已验证）全部可追溯，完整套件无 ResourceWarning；wheel 与 sdist 的 companion metadata
...
记录了 stage-2 的 default-deny 授权预演（仅选出 `kimi-code` executor 与
`dsh-verifier` verifier，未启动外部进程）。这只证明授权预检，不替代真实
CLI 接力与验收闭环证据。
